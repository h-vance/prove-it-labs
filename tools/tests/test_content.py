...
class Demo(unittest.TestCase):
    """The animated demo must still show output the labs actually produce.

    `docs/demo.svg` is the first thing a stranger sees, in the README and in
    anything the repository gets shared into. It is generated by
    `site/scripts/demo-svg.mjs` from `docker/01`'s recorded transcript, which
    means it is checked at the moment it is built and then never again: it is
    a committed artifact, and a re-recording that changes what the system says
    would leave a confident animation showing output nothing produces any more.

    A demo is a claim about the product. This repository does not ship claims
    nothing checks, and a stale one on the front page would be the loudest
    possible version of the failure.

    The check reads what the SVG claims rather than being told: it pulls the
    commands out of the drawing, then holds each one to the transcript.
    """

    SVG = ROOT / "docs" / "demo.svg"
    TRANSCRIPT = (ROOT / "labs" / "docker" /
                  "01-service-unavailable-after-deploy" / "transcript.json")
    ...
    def setUp(self):
        if not self.SVG.is_file():
            self.fail(f"{self.SVG.relative_to(ROOT)} is missing. "
                      f"Run `npm --prefix site run demo`.")
        self.svg = self.SVG.read_text(encoding="utf-8")
        # Everything the drawing renders, with the markup removed, so a check
        # for a line of output does not have to know how it was split into
        # tspans by the highlighting.
        self.rendered = self.unescape(re.sub(r"<[^>]+>", "", self.svg))
        raw = json.loads(self.TRANSCRIPT.read_text(encoding="utf-8"))
        self.entries = raw if isinstance(raw, list) else raw.get("entries", [])

    def claimed_commands(self) -> list[str]:
        return [self.unescape(match) for match in
                re.findall(r'<tspan class="cmd">(.*?)</tspan>', self.svg, re.S)]

    def test_the_demo_shows_commands_that_were_really_recorded(self):
        claimed = self.claimed_commands()
        self.assertTrue(claimed, "The demo draws no commands at all, so it is not a terminal.")
        recorded = {entry["command"] for entry in self.entries}
        for command in claimed:
            with self.subTest(command):
                self.assertIn(
                    command, recorded,
                    f"docs/demo.svg types {command!r}, which is in no recording in "
                    f"{self.TRANSCRIPT.relative_to(ROOT)}. Regenerate it with "
                    f"`npm --prefix site run demo`.",
                )

    def drawn_output(self) -> list[str]:
        """Every line the drawing presents as machine output.

        Matched on the `out` class, which the generator puts only on recorded
        output: the command lines carry `cmd`, the window furniture carries
        `dim`, and the caption underneath the rule carries `note`. The caption
        is the course talking rather than the machine, and holding it to the
        transcript would be a category error.
        """
        found = []
        for block in re.findall(r'<text id="r\d+"[^>]*class="out">(.*?)</text>',
                                self.svg, re.S):
            line = self.unescape(re.sub(r"<[^>]+>", "", block)).strip()
            if line:
                found.append(line)
        return found

    def test_every_line_the_demo_shows_is_a_line_the_system_printed(self):
        """The first version of this checked only each command's opening line.

        `docker ps` opens with its column headers, so the check read
        `CONTAINER ID  IMAGE  ...` and was satisfied, while every row of actual
        container state underneath it went unexamined. Rewriting a container's
        status in the drawing passed. That is the same shape as the frontmatter
        links in C7: a check existed, and the interesting input never reached
        it.

        Every drawn line is now held to the recording, not just the first.
        """
        drawn = self.drawn_output()
        self.assertGreater(len(drawn), 2,
                           "The demo draws almost no output, so this proves nothing.")
        recorded = [line.strip() for entry in self.entries
                    for line in entry["output"].split("\n") if line.strip()]

        for line in drawn:
            # Wide table rows are truncated on purpose and say so with an
            # ellipsis, so a drawn line has to be the start of a recorded one
            # rather than equal to it.
            stem = line[:-3].rstrip() if line.endswith("...") else line
            with self.subTest(stem[:60]):
                self.assertTrue(
                    any(candidate.startswith(stem) for candidate in recorded),
                    f"docs/demo.svg draws a line no recording produced:\n"
                    f"  {line}\n"
                    f"The demo is generated from labs/docker/01's transcript, so "
                    f"either the recording moved and the drawing did not, or the "
                    f"drawing was edited by hand. Regenerate it with "
                    f"`npm --prefix site run demo`.",
                )
